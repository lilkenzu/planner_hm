...
## Ограничение этого файла

Скриншоты интерфейса, которые агент показывал по ходу разработки (шаги 1–7),
демонстрировались прямо в диалоге и разбирались там же, но не сохранены как
файлы в репозиторий — у используемого инструмента браузера нет прямого
способа выгрузить снимок на диск в файловую систему проекта. Конкретные
значения из этих снимков (проценты, время, тексты ошибок) зафиксированы
текстом в `sessions/session-1.md` на каждом шаге. Воспроизвести визуально —
`python3 -m http.server 8765`, меньше минуты.
